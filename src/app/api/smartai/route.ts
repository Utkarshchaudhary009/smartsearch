...
8. In the last, provide all sources with their links formatted in proper markdown. Ensure that only the links are generated, with appropriate link text. Present these links as badges with minimal spacing, and utilize dropdowns to display the sources for an enhanced user experience.
chat history: ${chatHistory
      .map((message) => `${message.role}: ${message.content}`)
      .join("\n")}
User message: ${userMessage}
