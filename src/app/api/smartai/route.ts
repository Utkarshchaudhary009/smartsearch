...
8. in the last give all the sources with there link in beautill ui that inhances users ux. dont give in list foem buut bandges. with liitle space and use dropdown to show sources.
chat history: ${chatHistory
      .map((message) => `${message.role}: ${message.content}`)
      .join("\n")}
User message: ${userMessage}
